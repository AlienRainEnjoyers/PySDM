...
# load results
dir = 'output/fig02/'
RHs = np.load(dir+'RHs.npy')
r0grid = np.load(dir+'r0grid.npy')
RHgrid  = np.load(dir+'RHgrid.npy')
...
c_10 = axs[1,0].contour(r0grid*1e3, RHgrid,m_frac_evap,colors='indigo',linewidths=1,linestyles='--',levels=[0.1])
cb.add_lines(c_10)
...
axs[1,0].annotate(text=r'$r_\\mathrm{min}$',xy=(0.35,0.27),c='darkviolet',backgroundcolor='w',size=8)
...
plt.savefig('figs/fig02.pdf',transparent=True,bbox_inches='tight',pad_inches=0.5)
plt.close()
