...
class TestNPYComparison:
    ...
    def _calculate_cloud_properties(self, planet: EarthLike, surface_RH: float, formulae_instance: Formulae):
        ...
        def solve_Tcloud(T_candidate):
            pv_ad = initial_water_vapour_mixing_ratio / (initial_water_vapour_mixing_ratio + const.eps)*planet.p_STP*(T_candidate/planet.T_STP)**(cp_mix/R_air_mix)
    ...
    def test_figure_2_replication_accuracy(self):
        ...
        with GroundTruthLoader(groundtruth_dir) as gt:
            ...
            all_indices = np.array([(i, j) for i in range(n_rh_values) for j in range(n_radius_values)])
            ...
            for i_rh, j_r in sampled_ij_pairs:
                ...
                if np.isnan(expected_m_frac_evap):
                    ...
                    assert not np.isnan(simulated_m_frac_evap_point), \
